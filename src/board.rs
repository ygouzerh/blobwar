...
impl Board {
    ...
    fn fill_individual_neighbours(&mut self) {
        for position in 0i8..64i8 {
            ...
            let y = position / 8;
            let ok = 0..8;
            for neighbouring_x in (x - 2)..(x + 3) {
                if ok.contains(neighbouring_x) {
                    for neighbouring_y in (y - 2)..(y + 3) {
                        if ok.contains(neighbouring_y) {
                            let distance =
                                max((neighbouring_x - x).abs(), (neighbouring_y - y).abs())
                                    as usize;
                            if distance != 0 {
                                let coordinate =
